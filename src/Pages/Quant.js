    ...
    <div className="flex">
      ...
      <div className="flex-grow py-10 md:px-20 px-10">
        ...
        <div className="mt-2 flex gap-8 pt-10 lg:flex-row md:flex-col">
          ...
          Quantitative Aptitude is that one integral, and inseparable part of aptitude exams in India. Quantitative aptitude tests the number handling techniques and problem-solving skills of an individual. It is an important requisite in clearing the preliminary stages of several exams, especially the Management courses. The whole purpose of Quantitative Aptitude is to evaluate the thinking process along with the analytical skills of the aspirants. It is not just the ability to solve numerical problems, the candidates require a lot of logical thinking to crack Quantitative Aptitude
          </p>
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-1 gap-6 justify-between mt-12">   
          ...
          <RatingCard serialNo={9} Title="Geometry" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />
          <RatingCard serialNo={10} Title="Coordinate Geometry" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />
          <RatingCard serialNo={11} Title="Trigonometry" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />                
          <RatingCard serialNo={12} Title="Numbers" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />                
          <RatingCard serialNo={13} Title="Simple, Special & Quadratic Equation" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />          
          <RatingCard serialNo={14} Title="Sequence & Series" currentChapter={ratingResponse.filter((el) => {return el.chapter === "Time, Speed & Distance"})} />
          </> }
          ...
          <RatingCard serialNo={11} Title="Trigonometry" currentChapter={[]} />                
          <RatingCard serialNo={12} Title="Numbers" currentChapter={[]} />
          <RatingCard serialNo={13} Title="Simple, Special & Quadratic Equation" currentChapter={[]} />
          <RatingCard serialNo={14} Title="Sequence & Series" currentChapter={[]} />
          </> }
        </div>
      </div>
    </div>
